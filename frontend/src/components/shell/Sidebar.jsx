    ...
    <aside className={`hidden md:flex flex-col bg-surface border-r border-border transition-all duration-200 ${collapsed ? "w-16" : "w-56"} shrink-0`}>
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {items.map((item) => {
          ...
          if (item.soon) return (
            <div key={item.label} className="flex items-center gap-3 px-2.5 py-2 rounded-lg text-sm text-muted/40 cursor-default">
              <Icon className="w-4 h-4 flex-shrink-0" />
              {!collapsed && <span className="flex-1 truncate">{item.label}</span>}
              {!collapsed && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-bg border border-border text-muted/50">Soon</span>}
            </div>
          );
